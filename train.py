...
def proper_cube_rotations() -> List[Dict[str, object]]:
        ...
        for sx in (-1, 1):
            for sy in (-1, 1):
                for sz in (-1, 1):
                    ...
                    flips = tuple(
                        i for i, s in enumerate((sx, sy, sz))
                        if s == -1
                    )
    ...
    if len(rots) != 24:
        raise RuntimeError(f"Expected 24 rotations; got {len(rots)}")
...
def apply_spatial_rotation_tensor(
    x: torch.Tensor,
    rot: Dict[str, object],
) -> torch.Tensor:
    ...
    if x.ndim == 4:
        ...
        raise ValueError(
            f"Expected 4D or 5D tensor; got {tuple(x.shape)}"
        )
...
def add_gradient_channel(
    ...
    if x.ndim != 5 or x.shape[1] != 1:
        raise ValueError(
            f"Expected [B,1,X,Y,Z]; got {tuple(x.shape)}"
        )
    ...
    std = flat.std(dim=1).view(-1, 1, 1, 1)
...
def find_data_pairs(
    ...
    image_files = sorted(
        glob.glob(os.path.join(img_dir, "*.nii.gz"))
    )

    mask_files = sorted(
        glob.glob(os.path.join(mask_dir, "*.nii.gz"))
    )
    ...
    for img in image_files:
        ...
        if (
            base.startswith("._")
            or "mask" in base.lower()
            or "normcoord" in base.lower()
        ):
            continue
        ...
        if len(candidates) > 1:
            print(
                f"[WARN] Multiple masks found for {img}; "
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Orientation-agnostic rodent brain extraction training."
    )
    ...
    parser.add_argument(
        "--wandb_project",
        default="orientation-agnostic-mbe",
    )
    ...
    if len(data_dicts) < 2:
        raise RuntimeError(
            f"Need at least 2 pairs; found {len(data_dicts)}"
        )
    ...
    msd_metric = SurfaceDistanceMetric(
        include_background=False,
        symmetric=True,
        reduction="mean",
    )
    ...
    metadata_path = ckpt_path.with_suffix(
        ckpt_path.suffix + ".json"
    )
    ...
    for epoch in range(1, args.training_epochs + 1):
        ...
        pbar = tqdm(
            train_loader,
            desc=f"Epoch {epoch}/{args.training_epochs} [Train]",
        )
        ...
        for batch in pbar:
            ...
            with torch.amp.autocast(
                ...
                if outputs.shape[2:] != y.shape[2:]:
                    ...
                    raise RuntimeError(
                        f"Mismatch: outputs={tuple(outputs.shape)} "
                        f"labels={tuple(y.shape)}"
                    )
        ...
        with torch.no_grad():
            ...
            vbar = tqdm(
                val_loader,
                desc=f"Epoch {epoch}/{args.training_epochs} [Val]",
            )
            ...
            for batch in vbar:
                ...
                with torch.amp.autocast(
                    ...
                    if val_outputs.shape[2:] != val_y.shape[2:]:
                        ...
                        raise RuntimeError(
                            f"Val mismatch: "
                            f"outputs={tuple(val_outputs.shape)} "
                            f"labels={tuple(val_y.shape)}"
                        )
        ...
        mean_hd95 = float(
            hd95_metric.aggregate().item()
        )

        mean_msd = float(
            msd_metric.aggregate().item()
        )

        mean_sens = float(
            sens_metric.aggregate()[0].item()
        )
        ...
        is_best = (
            mean_dice > best_val_dice
        ) or (
            math.isclose(mean_dice, best_val_dice)
            and mean_hd95 < best_val_hd95
        )
        ...
        if is_best:
            ...
            print(
                ...
                f"HD95={best_val_hd95:.4f}"
            )

        if args.save_last:

            last_path = ckpt_path.with_name(
                ckpt_path.stem + "_last" + ckpt_path.suffix
            )
